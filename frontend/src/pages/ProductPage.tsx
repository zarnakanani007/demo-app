...
const ProductsPage: React.FC = () => {
  ...
  const validationSchema = Yup.object({
    ...
    category: Yup.string()
      .oneOf(categories.filter(cat => cat !== 'all'), "Invalid category")
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="p-6 bg-white rounded-2xl shadow-xl border border-gray-200">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800 cursor-pointer">Products Management</h1>
          <button
        ...
        {loading ? (
          <div className="flex justify-center items-center py-8">
            ...
            <span className="ml-2 text-gray-600 cursor-pointer">Loading products...</span>
          </div>
                ...
                {filteredProducts.map((p, i) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <img src={p.image} alt={p.name} className="w-16 h-16 object-cover rounded cursor-pointer" />
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 font-medium cursor-pointer">{p.name}</td>
                    <td className="px-6 py-4">
                      <span className="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full capitalize cursor-pointer">
                        {p.category}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 cursor-pointer">${p.price}</td>
                    <td className="px-6 py-4">
      ...
      {editProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4 cursor-pointer">Edit Product</h2>
